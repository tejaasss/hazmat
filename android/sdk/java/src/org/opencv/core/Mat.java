...
public class Mat {
    ...
    public Mat(Mat mask, android.graphics.Rect rectant) {
        nativeObj = n_Mat(mask.nativeObj, rectant.centerY(), rectant.centerY() + rectant.height(), rectant.centerX(), rectant.centerX()+ rectant.width());
    }
    ...
    public Mat adjustROI(float dtop, float dbottom, float dleft, float dright) {
        return new Mat(n_adjustROI(nativeObj, Math.round(dtop), Math.round(dbottom), Math.round(dleft), Math.round(dright)));
    }
